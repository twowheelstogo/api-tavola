...
export default async function addCartCatalogs(context, cart, input, options = {}) {
  ...
  const updatedItemList = (cart.items || []).slice(0);

  const currentDateTime = new Date();

  const promises = input.items.map(async (inputItem) => {
    ...
    const currentMatchingItemIndex = cart.items.findIndex(
  ...
  console.info("addCartCatalogs catalogs", input.catalogs);
  console.info("addCartCatalogs items", input.items);

  return { incorrectPriceFailures, minOrderQuantityFailures, updatedItemList, updated };
}
